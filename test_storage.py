...
class Test(unittest.TestCase):
    ...
    def test_load_todays_company_names(self):
        ...
        company_names_empty = storage.load_todays_company_names(
            storage_directory
        )
    ...
    def test_store_company_data_invalid(self):
        # invalid scrape_type
        self.assertRaises(AssertionError, storage._store_company_data,
                          None, storage_directory, "foo")
        # invalid company_data
        self.assertRaises(AssertionError, storage._store_company_data,
                          [1], storage_directory, "names")
        self.assertRaises(AssertionError, storage._store_company_data,
                          {"1":"name"}, storage_directory, "names")
        self.assertRaises(AssertionError, storage._store_company_data,
                          {1:2}, storage_directory, "names")
        self.assertRaises(AssertionError, storage._store_company_data,
                          [1], storage_directory, "metrics")
        self.assertRaises(AssertionError, storage._store_company_data,
                          [Company(c_id=1, c_name="bar")],
                          storage_directory, "metrics")
